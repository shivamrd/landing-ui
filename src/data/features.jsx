export const features = [
  {
    title: "Complex Layout Extraction",
    description: [
      "Parse documents into semantic chunks to ensure high-quality data ingestion for RAG workflows.",
      "Zero-shot parsing of PDFs, scans, and tables without layout-specific training.",
      "Capture semantic relationships beyond OCR including form fields and layouts."
    ],
    image: "/assets/images/Chart-table-image.webp"
  },
  {
    title: "Agentic Document Understanding",
    description: [
      "Autonomously reason over documents using agents.",
      "Extract structured insights from complex real-world data.",
      "Built-in auditability and traceability."
    ],
    image: "/assets/images/Complex-Layout-Extraction.webp"
  },
  {
    title: "Production-Ready Accuracy",
    description: [
      "Enterprise-grade reliability with measurable accuracy.",
      "No model training required.",
      "Designed for real-world deployment at scale."
    ],
    image: "/assets/images/visual_grounding-1.webp"
  }
];
